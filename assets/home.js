...
const topicRequest = encodeURIComponent('https://example.com/AcceptedTeamRequest-'+idUser);
const eventSourceRequest = new EventSource(`http://localhost:32768/.well-known/mercure?topic=${topicRequest}`);

eventSourceRequest.onmessage = function(event) {
    console.log("laa");
    console.log(event.data);
    let data = JSON.parse(event.data);
    window.location.href = '/lobby/'+data.toString();
};
...
const updateAvatar = document.querySelector('#update_user_form_avatar');
const voile = document.querySelector('.voile');
const img = document.querySelector('.avatar-update');
...
if(voile){
    ...
    updateAvatar.addEventListener('change', () => {
