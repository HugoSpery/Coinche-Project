...
export default class extends Controller {
    connect() {
        ...
        this.seeRequest = document.querySelector(".see-request");
        this.optionsFriend = document.querySelectorAll(".option-friends");
        this.addFriend = document.querySelector(".add-friend");
        this.seeFriend = document.querySelector(".see-friend");
        ...
        this.closeMenu.addEventListener("click", () => {
            ...
            setTimeout(() => {
                this.paramMenu.style.display = "none";
            }, 500);
        ...
        if (this.seeRequest){
            this.seeRequest.addEventListener("click", () => {
                this.optionsFriend.forEach(option => {
                    option.classList.remove("active")
                });
                this.seeRequest.classList.add("active");
                this.url.searchParams.set("addFriend", "");
                window.history.replaceState({}, "", this.url);
        ...
        if(this.addFriend){
            console.log("laa");
            this.addFriend.addEventListener("click", () => {
                this.optionsFriend.forEach(option => {
                    option.classList.remove("active")
                });
                this.addFriend.classList.add("active");


                this.url.searchParams.set("addFriend", "open");
        ...
        if (this.seeFriend){
            this.seeFriend.addEventListener("click", () => {
                this.optionsFriend.forEach(option => {
                    option.classList.remove("active")
                });
                this.seeFriend.classList.add("active");
                this.url.searchParams.set("addFriend", "");
                window.history.replaceState({}, "", this.url);
